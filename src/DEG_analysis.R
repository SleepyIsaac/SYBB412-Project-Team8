...
highlight_genes <- c("FOLR2", "APOE", "MMP2", "IFNG", "CXCL9", "CXCL10", "PDCD1", "LAG3", "IDO1", "COL1A1", "SERPINE1", "PLAUR")
deg_results$label <- ifelse(deg_results$gene %in% highlight_genes, deg_results$gene, NA)
...
volcano_plot <- ggplot(deg_results, aes(x = logFC, y = neglogFDR, color = Significant)) +
  ...
  geom_text_repel(aes(label = label), size = 2, max.overlaps = 100) +
  facet_wrap(~ paste(cluster_id, contrast), scales = "free") +
  theme_minimal() +
  scale_color_manual(values = c("grey", "red")) +
  labs(y = "-log10 FDR", x = "log2 Fold Change", color = "Significant")
